...
<template>
  <div
    class="vf-node vf-node-send rounded-lg border-2 bg-emerald-50 dark:bg-emerald-950/40 shadow-sm"
  ...
  >
    ...
    <div class="flex items-start gap-2 p-3">
      <div class="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded bg-white shadow-sm dark:bg-neutral-800">
        <PaperAirplaneIcon class="h-4 w-4 text-emerald-700" />
      </div>
      <div class="min-w-0 flex-1">
        <div class="flex items-center gap-1.5">
          <span class="text-[10px] font-semibold uppercase tracking-wide text-emerald-900 dark:text-emerald-200">Send</span>
          ...
          <span v-if="data.delay_minutes > 0" class="rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-mono text-amber-800 dark:bg-amber-900/40 dark:text-amber-200">+{{ data.delay_minutes }}m</span>
        </div>
